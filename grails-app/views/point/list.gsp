			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="name" title="${message(code: 'point.name.label', default: 'Name')}" />
					
						<g:sortableColumn property="type" title="${message(code: 'point.type.label', default: 'Type')}" />
				...
				<g:each in="${pointInstanceList}" status="i" var="pointInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${pointInstance.id}">${fieldValue(bean: pointInstance, field: "name")}</g:link></td>
					
						<td>${fieldValue(bean: pointInstance, field: "type")}</td>
